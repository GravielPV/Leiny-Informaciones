...
-- 4. Política para que todos puedan leer videos habilitados (reemplazar si existe)
DROP POLICY IF EXISTS "Anyone can view enabled live videos" ON live_videos;
CREATE POLICY "Anyone can view enabled live videos"
ON live_videos FOR SELECT
TO public
USING (is_enabled = true);

-- 5. Política para que usuarios autenticados puedan gestionar videos (reemplazar si existe)
DROP POLICY IF EXISTS "Authenticated users can manage live videos" ON live_videos;
...
-- 7. Trigger para actualizar timestamp (reemplazar si existe)
DROP TRIGGER IF EXISTS update_live_videos_updated_at_trigger ON live_videos;
